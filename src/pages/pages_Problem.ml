...
2 4,5|j}];
      h2 [s {j|既知のバグ|j}];
      p [s {j|最初からクリア状態（歯車全てが上向き）の場合、本来何も入力しなくてもクリアできるはずですが、現状ではその入力は不正扱いになってしまいます。|j}];
      p [s {j|リセットして状態を変える等、対応をお願いします。|j}];
    ]
}
